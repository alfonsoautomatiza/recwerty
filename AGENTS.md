...
- The manifest is staged at `docs/es/updates/manifest-stable.json` but the default-language build (es → site root) serves it at `/updates/`. The hook `hooks/manifest_mirror.py` (registered in `mkdocs.yml`) duplicates it to `/es/updates/` so the baked URL resolves. NEVER remove the hook or the mirror entry. The private repo's `c/product.json` must keep `manifest_url` pointing to the `/es/updates/` path on `alfonsoautomatiza.github.io`.
